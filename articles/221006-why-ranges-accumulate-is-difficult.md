...
STL には異なる 2 つの型が等値比較可能であることを表すコンセプトとして、`equality_comparable_with` が用意されています。このコンセプトは 2 つの型が共通型であることを要求します。一方、`equality_comparable_with` を用いて制約された STL のアルゴリズムは現時点ではありません。<!-- TODO: 現時点、はいつを指している? -->
